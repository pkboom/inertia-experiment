...
export default {
  ...
  saveScrollPositions() {
    this.replaceState({
      ...
      scrollRegions: Array.prototype.slice
        .call(this.scrollRegions())
        .map(region => {
          return {
  ...
  visit(
    ...
    if (
      onStart(visit) === false ||
      !this.fireEvent('start', { cancelable: true, detail: { visit } })
    ) {
      return
    }
    ...
    return new Proxy(
      Axios({
        ...
        .then(response => {
          ...
          return this.setPage(response.data, {
            visitId,
            replace,
            preserveScroll,
            preserveState,
        ...
        .catch(error => {
          if (
            this.fireEvent('error', { cancelable: true, detail: { error } })
          ) {
            return Promise.reject(error)
          }
        })
    ...
    return Promise.resolve(this.resolveComponent(page.component)).then(
      component => {
        if (visitId === this.visitId) {
          ...
          replace
            ? this.replaceState(page, preserveState)
            : this.pushState(page)
    ...
    window.history.replaceState(
      {
        ...{
          cache:
            preserveState && window.history.state
              ? window.history.state.cache
              : {},
        },
        ...page,
      },
  ...
  restoreState(event) {
    if (event.state) {
      ...
      let visitId = this.createVisitId()
      return Promise.resolve(this.resolveComponent(this.page.component)).then(
        component => {
          if (visitId === this.visitId) {
            this.updatePage(component, this.page.props, {
              preserveState: false,
            }).then(() => {
